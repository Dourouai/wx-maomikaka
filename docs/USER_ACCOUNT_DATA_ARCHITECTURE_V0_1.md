...
> 状态：已落地第一版同步链路，三个数据云函数已部署到 CloudBase 开发环境；待小程序真机完成账号绑定、同步和分享链路联调。
>
...
以下三个云函数已上传并部署到开发环境，当前状态均为“正常”：

- `auth-bootstrap`
- `sync-guest-data`
- `cat-archive-share`

先在 CloudBase 文档数据库中创建 `users`、`cat_profiles`、`encounters`、`cat_shares` 四个集合；代码会把没有记录的集合查询当作空结果，但首次写入仍需要集合已存在。
...
1. 本版实现的是“数据绑定与同步”，不是完整的服务端拍摄结算系统。
2. 当前 `catProfileId` 是用户维度的图鉴角色档案，不宣称识别出了现实中的同一只猫。
3. `daily_usage`、服务端扣罐、正式奖励结算和处理任务恢复仍应作为下一阶段服务端化工作。
