...
拍照 → 压缩到微信图片安全接口限制内
     ...
     → 保留通过检测的 fileID，cat-vision 与 cat-transform 并行处理
     → 两条链路都完成后，确认单猫并保存主体图
     → 保存和分享
```

内容安全通过后，已经上传的安全图片 `fileID` 会同时交给 `cat-vision` 和 `cat-transform`，避免重复上传。两条链路都完成后，页面才一次性展示最终主体图和评分；如果视觉识别确认不是单猫，或任一服务失败，则不写入本地拍摄记录，并清理共享输入及可能生成的主体图。主体处理只用于保留猫咪主体，不生成卡片文案、故事或装饰；当前模型结果不再进行二次图片安全检测，页面不展示模型生成说明。检测服务不可用时采用失败关闭，避免通过异常流程绕过审核。
