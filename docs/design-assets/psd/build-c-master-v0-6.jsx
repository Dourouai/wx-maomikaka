...
// L5 为顶部唯一的动态文字与数据智能对象。所有固定图形同时登记为可编辑路径。
...
var CORE_L4 = [
  "L4-01-frame-primary-line-C.png", "L4-04-frame-highlight-line-C.png",
  "L4-07-photo-recess-deep-C.png", "L4-10-photo-aperture-primary-C.png",
  "L4-20-number-fill-C.png", "L4-21-number-primary-line-C.png",
  "L4-26-score-fill-C.png", "L4-27-score-primary-line-C.png",
  "L4-32-name-divider-C.png",
  "L4-41-metric-module-fill-C.png", "L4-42-metric-cell-1-fill-C.png", "L4-43-metric-cell-2-fill-C.png",
  "L4-44-metric-cell-3-fill-C.png", "L4-45-metric-module-primary-line-C.png",
...
var L1_CORE = [
  "L1-01-outer-gradient-C.png", "L1-02-shell-inner-C.png", "L1-03-surface-C.png",
  "L1-04-panel-C.png", "L1-05-panel-inner-C.png", "L1-06-photo-shadow-C.png",
  "L1-07-paper-highlight-C.png"
];
var L4A_SMALL = [
  "L4A-small-02-label-fill-C.png",
  "L4A-small-03-label-primary-line-C.png", "L4A-small-04-label-inner-highlight-C.png"
];
...
function importExact(names, parent, displayPrefix, visible) {
  for (var i = names.length - 1; i >= 0; i--) {
    var file = new File(baseRoot + names[i]);
    importRaster(file.fsName, displayPrefix + "｜" + names[i].replace(/\.png$/i, ""), parent, visible);
  }
}
...
function addEditablePaths(specs) {
  ...
  for (var i = 0; i < paths.length; i++) {
    ...
    var used = name.indexOf("C-L1-") === 0
      || name === "C-L4-photo-recess"
      || name === "C-L4-photo-aperture"
      || name === "C-L4-label-plate"
      || name === "C-L4-label-plate-inner"
      || name === "C-L4-number-plate"
      || name === "C-L4-number-plate-inner"
      || name === "C-L4-score-seal"
      || name === "C-L4-score-seal-gold"
      || name === "C-L4-name-divider"
      || name === "C-L4-metric-module"
      || name === "C-L4-metric-module-inner"
      || name === "C-L4-metric-cell-1"
      || name === "C-L4-metric-cell-2"
      || name === "C-L4-metric-cell-3"
      || name === "C-L4-metric-divider-1"
      || name === "C-L4-metric-divider-2"
      || name.indexOf("C-L4-icon-") === 0;
    if (used) addPathItem("可编辑路径｜" + name, paths[i].points, paths[i].closed);
  }
}
...
function addDynamicTextSource() {
  ...
  addText("编号值｜No.后对齐｜MK-260903-000013", "MK-260903-000013", 442, 129, 18, ink, source, latinFont, false);
  addText("宠物名｜最多5字｜左对齐", "桃桃晒太阳", 104, 1028, 38, ink, source, chineseFont, false);
  addParagraphText("相遇文案｜最多50字｜左对齐", "午后的阳光刚刚好，\n遇见一只安静的猫。", 104, 1110, 415, 70, 19, ink, source, chineseFont);
  addText("咪咔分数｜0-100｜占位100", "100", 579, 1075, 38, ink, source, latinFont, true);
  addText("魅力分数｜0-100｜占位100", "100", 176, 1278, 30, ink, source, latinFont, true);
  addText("机灵分数｜0-100｜占位100", "100", 379, 1278, 30, ink, source, latinFont, true);
  addText("灵气分数｜0-100｜占位100", "100", 582, 1278, 30, ink, source, latinFont, true);
...
function buildCMaster() {
  doc = app.documents.add(720, 1420, 72, "C-master-v0-6-editable", NewDocumentMode.RGB, DocumentFill.TRANSPARENT);

  // Groups are added bottom-to-top. Photoshop adds each new group at the top.
  var l1 = addGroup("L1｜母版底纸｜固定｜底部");
  importExact(L1_CORE, l1, "L1｜固定底纸", true);
  ...
  var number = addGroup("03｜右上编号牌｜独立组件｜底色、描边、No. 与编号值对齐", l4);
  ...
  addText("固定文字｜No.｜左右留白校准", "No.", 405, 129, 17, "3D5241", number, "Baskerville", false);
  addText("固定文字｜MIKA", "MIKA", 578, 1029, 15, "3D5241", score, "Baskerville", true);

  // The C badge is an independent small L4A component. Its source art and
  // path coordinates are both scaled to 72% around the original anchor.
  var label = addGroup("L4A｜等级标签 C｜缩小版｜独立组件", l4);
  importExact(L4A_SMALL, label, "L4A｜C标签核心", true);
  addText("固定文字｜等级字母 C｜标签内对齐", "C", 72, 121, 43, "3D5241", label, "Baskerville", false);
  ...
  doc.saveAs(new File(baseRoot + "cat-card-c-master-v0-6-editable.psd"), options, true, Extension.LOWERCASE);
  ...
  doc.exportDocument(new File(baseRoot + "C-master-preview-v0-6-photoshop.png"), ExportType.SAVEFORWEB, previewOptions);
