...
function addDynamicTextSource() {
  ...
  addText("编号值｜No.后对齐｜MK-260903-000013", "MK-260903-000013", 438, 129, 18, ink, source, latinFont, false);
  addText("宠物名｜最多5字｜左对齐", "桃桃晒太阳", 104, 1000, 40, ink, source, chineseFont, false);
  addParagraphText("相遇文案｜最多50字｜左对齐", "午后的阳光刚刚好，\n遇见一只安静的猫。", 104, 1080, 415, 99, 21, ink, source, chineseFont);
...
function buildCMaster() {
  doc = app.documents.add(720, 1420, 72, "C-master-v0-5-editable", NewDocumentMode.RGB, DocumentFill.TRANSPARENT);
  ...
  addText("固定文字｜No.｜左侧基线", "No.", 401, 129, 17, "3D5241", number, "Baskerville", false);
  addText("固定文字｜MIKA", "MIKA", 578, 990, 16, "3D5241", score, "Baskerville", true);

  // v0.5: restore a restrained background label for the fixed rarity letter.
  // The label remains an independent L4A component so all five masters can
  // share the same geometry while changing only rarity styling.
  var label = addGroup("L4A｜等级标签 C｜独立组件｜背景、描边、字母", l4);
  var labelFiles = sortedPngs("L4A-", ["composite", "standalone", "L4A-08-label-letter"]);
  var coreLabelFiles = [];
  var optionalLabelFiles = [];
  for (var lf = 0; lf < labelFiles.length; lf++) {
    if (contains(CORE_L4A, labelFiles[lf].name)) coreLabelFiles.push(labelFiles[lf]);
    else optionalLabelFiles.push(labelFiles[lf]);
  }
  importReverse(coreLabelFiles, label, "L4A｜C标签核心", true);
  importReverse(optionalLabelFiles, label, "L4A｜可选细节", false);
  addText("固定文字｜等级字母 C｜标签内对齐", "C", 83, 151, 60, "3D5241", label, "Baskerville", false);
  ...
  doc.saveAs(new File(baseRoot + "cat-card-c-master-v0-5-editable.psd"), options, true, Extension.LOWERCASE);
  ...
  doc.exportDocument(new File(baseRoot + "C-master-preview-v0-5-label-aligned.png"), ExportType.SAVEFORWEB, previewOptions);
