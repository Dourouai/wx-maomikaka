...
const PROMPT_VERSION = 'cat-subject-only-cloudbase-i2i-v10-identity-preserving-matting';
...
const SUBJECT_PROMPT = [
  '任务类型：猫咪主体识别、前景分离与透明抠图。不是文生图，不是风格转换，不是重绘，不是美化，不是海报或卡片设计。',
  '唯一主体：把输入照片中的这只猫作为唯一核心主体。先沿着原图识别猫的完整真实轮廓，再删除背景；原图是唯一事实来源，不要根据常见猫咪形象猜测或改造。',
  '完整保留：必须保留属于同一只猫的头部、身体、耳朵、眼睛、鼻子、嘴巴、胡须、毛发、前后四肢、爪子、尾巴和其他可确认的附属结构。细长、低对比、模糊、局部断开、靠近画面边缘或被遮挡但能确认属于猫的部分，也必须保留，不得当成阴影、地面纹理或背景删除。',
  '尾巴优先检查：只要原图能看到全部或部分尾巴，就完整保留可见尾巴，并按原图的方向、花色、毛发质感和遮挡关系连接到身体。不得删除模糊尾巴，不得生成无尾猫，不得擅自改变尾巴长度、方向或形态。',
  '身份与姿态：严格保持原猫的身份、品种特征、脸部特征、毛色、花纹、眼睛颜色、毛发质感、身体比例、姿态、视角、光照和真实细节。不要换成另一只猫，不要默认改成正面坐姿，不要改变朝向、四肢数量、身体结构或表情。',
  '缺失部分：默认不补齐被裁切、完全遮挡或原图不存在的部分。只有在可见轮廓、姿态、透视、花色和光影能够可靠推断时，才允许以最小幅度延续原猫；无法确定时保留原始状态。严禁添加多余的腿、爪子、耳朵、尾巴或重复身体。',
  '背景清理：移除所有不属于猫的内容，包括地面、墙面、树枝、树叶、植物、人物、物体、文字、边框、贴纸、相框、卡片、装饰、背景图案和自然投影。自然阴影默认移除，除非它实际属于猫的毛发或身体轮廓。',
  '边缘处理：精细保留猫毛、胡须、耳朵尖、爪子、尾巴和半透明细节；去除背景残色、白边、黑边、光晕、溢色、锯齿和抠图孔洞。不要用纯色填充被移除的背景。',
  '输出要求：保持原图画布比例、猫咪在画布中的位置和完整可见范围，不得裁切猫。输出高分辨率 PNG，并使用真正的 alpha 透明通道；猫以外的像素必须完全透明，不得输出白色、黑色、灰色、彩色或棋盘格背景。不要添加任何文字、Logo、水印或模型标识。',
  '最终自检：确认尾巴、四肢、爪子、耳朵、胡须和其他可见猫咪结构均未误删；没有多余或重复部件；没有残留背景、阴影、边缘光晕、文字或装饰。只返回处理后的猫咪透明 PNG。',
].join(' ');
